...
full_m_f <- brm(
  ...
  data = all_data, # estava all_data e mudei (?)
  prior = priors,
  chains = 4,
  cores = 4,
  seed = 2912,
...
```{r}
# using unique point and line range
et_by_group_all |> 
  filter(ET_target == "ET80") |> 
  mutate(population = case_when(
  species == "Palmer amaranth" & location == "Lubbock" ~ "PA-LBK",
  ...
  mutate(group = factor(population, levels = c("PA-CC", "PA-CS", "PA-LBK", "WH-CC", "WH-CS", "WH-LBK"))) %>%
  ggplot(aes(x = reorder(group, median_ET), y = median_ET, color = group)) +
  geom_point(size=3) +
  geom_linerange(aes(ymin = lower_CI, ymax = upper_CI), size=1.2) +
  geom_text(aes(label=round(median_ET,1)),
            vjust=-1.1
            ) +
  coord_flip() +
  labs(
...
ggsave("figures/et80_point.png", width = 6, height = 5)
```
...
ggsave("figures/et50_point.png", width = 6, height = 5)
```
...
et_by_group_all |> 
  ...
  labs(
    x = NULL,
    y = "Months after burial for 80% seed viability (ET1)"
  ) +
  theme_foundation() +
  scale_color_manual(values = pop_colors) +
  scale_fill_manual(values = pop_colors) +
...
ggsave("figures/et1_point.png", width = 6, height = 5)
```
...
# --- Create heatmap plot ---
heatmap_data80 |> 
  mutate(Group1= factor(Group1, levels=c("WH-CC", "PA-CS", "WH-CS", "PA-CC", "WH-LBK", "PA-LBK")),
         Group2 = factor(Group2, levels=c("PA-LBK", "WH-LBK", "PA-CC", "WH-CS", "PA-CS", "WH-CC"))
         ) |> 
  ...
  labs(
    x = "Treatment compared against (ET80)",
    y = "Treatment being evaluated (ET80)"
  ) -> fig_et_80_pair
...
# --- Create heatmap plot ---
heatmap_data50 |> 
  mutate(Group1= factor(Group1, levels=c("WH-CC", "WH-CS", "PA-CS", "WH-LBK", "PA-CC", "PA-LBK")),
         Group2 = factor(Group2, levels=c("PA-LBK", "PA-CC", "WH-LBK", "PA-CS", "WH-CS",  "WH-CC"))
         ) |> 
ggplot(aes(x = Group2, y = Group1)) +
  geom_point(aes(color = Pr_G1_gt_G2, size = Pr_G1_gt_G2), shape = 16) +
  scale_color_gradient2(
...
# --- Create heatmap plot ---
heatmap_data1 |> 
  mutate(Group1= factor(Group1, levels=c( "WH-LBK", "PA-LBK", "WH-CS", "WH-CC", "PA-CS", "PA-CC")),
         Group2 = factor(Group2, levels=c("PA-CC", "PA-CS", "WH-CC", "WH-CS", "PA-LBK", "WH-LBK"))
         ) |> 
  ggplot(aes(x = Group2, y = Group1)) +
  geom_point(aes(color = Pr_G1_gt_G2, size = Pr_G1_gt_G2), shape = 16) +
  scale_color_gradient2(
  ...
  labs(
    x = "Treatment compared against (ET1)",
    y = "Treatment being evaluated (ET1)"
  ) -> fig_et_1_pair
...
```{r}
# Create individual columns
col1 <- fig_et_80_1 / fig_et_80_pair
col2 <- fig_et_50_1 / fig_et_50_pair
col3 <- fig_et_1_1 / fig_et_1_pair
...
# Save the plot
ggsave("figures/Final_figure_1.png", plot = final_plot, width = 16.5, height = 10)
```
